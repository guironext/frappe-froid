...
export type RequerantParDateRow = {
	recuId: string
	dat: string
...
export function RequerantsParDateTable({
	rows,
	totalMontant,
}: {
	rows: RequerantParDateRow[]
	totalMontant: number
}) {
	...
	function handleExportExcel() {
		...
		try {
			...
			XLSX.utils.book_append_sheet(workbook, worksheet, "Réquérants par date")

			const today = new Date()
			const yyyy = today.getFullYear()
			const mm = String(today.getMonth() + 1).padStart(2, "0")
			const dd = String(today.getDate()).padStart(2, "0")
			const filename = `requerants-par-date-${yyyy}-${mm}-${dd}.xlsx`
	...
	const grouped = useMemo(() => {
		const m = new Map<string, RequerantParDateRow[]>()
		for (const r of rows) {
			const arr = m.get(r.dat)
			if (arr) arr.push(r)
			else m.set(r.dat, [r])
		}
		...
		<div className="overflow-x-auto rounded-2xl border border-white/50 bg-white/50 shadow-inner">
			...
			<table className="w-full min-w-[min(100%,520px)] border-collapse text-left text-sm text-zinc-900">
				...
				<tbody>
					{grouped.map(([dat, rs]) => (
						<Fragment key={dat}>
							<tr key={`dat-${dat}`} className="border-b border-orange-100/80 bg-amber-50/60">
								<td colSpan={4} className="px-4 py-3 text-xs font-semibold uppercase tracking-wide text-orange-900">
									Date : {dat}
								</td>
							</tr>
							{rs.map((row) => (
									...
									<td className="px-4 py-3">
										<div className="flex items-center justify-center gap-2">
											...
											<Link
												...
												prefetch={false}
												className="hidden h-10 w-10 items-center justify-center rounded-xl border border-orange-200 bg-white/90 text-orange-700 shadow-sm transition hover:bg-amber-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-500"
											...
											<button
												...
												className="hidden h-10 w-10 items-center justify-center rounded-xl border border-red-200 bg-white/90 text-red-600 shadow-sm transition hover:bg-red-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-red-500 disabled:opacity-50"
					...
					<tr className="bg-white/70">
						<td className="px-4 py-3 font-semibold uppercase tracking-wide text-orange-900" colSpan={2}>
							Total
						</td>
						<td className="px-4 py-3 text-right tabular-nums font-semibold">
							{totalMontant.toLocaleString("fr-FR")} F CFA
						</td>
						<td className="px-4 py-3" />
					</tr>
				</tbody>
